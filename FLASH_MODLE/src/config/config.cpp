...
bool ModelConfig::has_evidence(const std::string& field) const
{
    const auto it = field_evidence.find(field);
    return it != field_evidence.end() &&
           !it->second.datasheet.empty() &&
           !it->second.page.empty() &&
