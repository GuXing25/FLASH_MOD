...
3. 构建对应的 FlashModel 运行时模型和内存模拟存储区。
4. Common Core 只承载 NOR/NAND 共享的稳定底层行为。
5. Command Facade 负责对外提供行为级命令入口。
6. Command Transaction Layer 负责描述命令的数据交换形态。
7. Chiplet Interface 负责把数据交换转换为芯粒间传输耗时。
8. Capability Modules 负责可选能力。
9. Device Policy 只处理少量难以配置化、模块化的厂商差异。
10. 仿真结束时可把主阵列和 OTP 区导出为二进制镜像，方便检查。
11. 每个 datasheet profile 都需要逐渐配套 golden behavior test。
