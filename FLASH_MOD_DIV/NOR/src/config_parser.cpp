...
void derive_profile(DeviceProfile& p)
{
    ...
    if (p.is_nand_like()) {
        throw std::runtime_error("NOR-only project only accepts NOR/SPI_NOR profiles");
    }
...
{
    const std::string v = upper(text);
    if (v == "NOR" || v == "SPI_NOR") return FlashClass::NOR;
    throw std::runtime_error("NOR-only project does not support FLASH_CLASS: " + text);
}
